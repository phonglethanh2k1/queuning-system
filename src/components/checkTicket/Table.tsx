...
const Tables = (props: Props): JSX.Element => {
  ...
  const getQuery = (equipment: CollectionReference<DocumentData>, filterValue?: CheckTicketStatus) => {
    if (!filterValue) {
  ...
  const renderGateCheck = (gateCheckin: GateCheckIn) => {
    if (gateCheckin === GateCheckIn.ONE) {
      return <Typography color="grey.600">{GateCheckOption[GateCheckIn.ONE].label}</Typography>;
    }

    if (gateCheckin === GateCheckIn.TWO) {
      return <Typography color="grey.600">{GateCheckOption[GateCheckIn.TWO].label}</Typography>;
    }
    return <Typography color="grey.600">{GateCheckOption[GateCheckIn.THREE].label}</Typography>;
  };
          ...
          <TableBody>
            {dataTable.map((row, index) => (
              <TableRow key={row.id}>
                ...
                <TableCell align="left">{renderGateCheck(row.gateCheckIn)}</TableCell>
